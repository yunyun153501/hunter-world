...
const RARE_FAMILY_PRESETS = {
  ...
  function parseMonsterMeta(entry) {
    ...
    if (prefixMatch) {
      ...
      if (st) {
        ...
        const defaultTurns = hardCcTypes.includes(st) ? 1 : dotTypes.includes(st) ? 3 : 2;
    ...
    if (!meta.onHitStatus && (kind === 'boss' || kind === 'elite') && meta.baseElement && meta.baseElement !== 'none') {
      ...
      if (elSt && normStatus(elSt)) {
        ...
        meta.onHitTurns = hardCcTypes.includes(elSt) ? 1 : dotTypes.includes(elSt) ? 3 : 2;
      }
    }
  ...
  function getStatusDefaultProfile(type) {
    ...
    const map = {
      ...
      burn:{ turns:3, chance:0.23, power:0 },
      curse:{ turns:1, chance:0.18, power:0 },
